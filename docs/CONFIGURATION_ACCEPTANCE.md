...
| 配置键 | 默认值 | 作用与边界 |
| --- | --- | --- |
| `ui_renderer` | `pillow` | Campaign、日程、联盟总览/记录/成员、Profile 与 Character 支持 `pillow` / `t2i`；缺失或未知值使用 Pillow。T2I 注入 AstrBot 原生 html_render，30 秒超时或失败后使用同一 DTO 回退 Pillow（记录、成员和日程回退原文本），不再次请求 API。原生渲染策略与端点由 AstrBot 管理。 |
