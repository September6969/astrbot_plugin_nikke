...
| PR | 分支 | 主题 | 当前状态 |
| --- | --- | --- | --- |
| #8 | `feat/union-raid-v2` | Union Raid Increment A | OPEN / Draft / CI green；head `4ad74c0`；CI `34190392211` 四项全绿（Node、Python 3.10–3.12） |
| #9 | `feat/announcement-v2` | Announcement Increment A + cache lifecycle P1 | OPEN / Draft / CI green；head `78b33a7`；CI `34191009192` 四项全绿（Node、Python 3.10–3.12） |
| #10 | `feat/dynamic-voice-v2` | Dynamic Voice Increment A | OPEN / Draft / CI green；head `6fc6137`；CI `34191491292` 四项全绿（Node、Python 3.10–3.12） |
...
| #21 | `feat/plugin-shutdown-lifecycle` | Plugin shutdown lifecycle idempotency | OPEN / Draft / CI green；head `080ea94`；CI `34172309963` |
| #22 | `feat/data-backup-hardening` | Offline database and secret.key backup hardening | OPEN / Draft / CI green；head `bc839c4`；CI `34186296724` |
...
| #34 | `feat/cdk-batch-contract-v1` | CDK batch idempotency contract reconciliation | OPEN / Draft / CI green；head `f07828e`；CI `34189931526` 四项全绿（Node、Python 3.10–3.12） |
...
| CDK batch idempotency contract reconciliation | `feat/cdk-batch-contract-v1` | `bada0b3` | Draft PR #34；确认主命令逐码复用 `action_runs`，run key 为 `cdk:{qq_id}:{game_uid}:{SHA256(code)}`，unknown/终态不自动重放，失败/过期可原子重领；新增命令层行为测试，锁定 Store、QQ ID、游戏 UID 接线及结果输出脱敏；本地专项 `test_cdk_persistence.py` 4 passed、全量 pytest 257 passed、43 subtests、2 warnings、Node 3 passed、compileall/diff check 通过；head `f07828e`；CI `34189931526` 四项全绿；未访问真实账号、未执行兑换或部署 |
...
| Campaign History numeric contract | `feat/campaign-history-contract-v2` | `bada0b3` | Draft PR #15；严格 tid/lv/combat/slot 数值与槽位合同，拒绝空白/带符号数字字符串；响应 `code` 只接受非布尔 JSON 整数，避免 `False`/浮点误入成功分支；非映射响应、缺失/非列表 `data.list`、非五人列表返回 ERROR，明确空列表保持 UNAVAILABLE，并对超长列表提前失败；本地 Python 3.10.11 主题相关 unittest 30 项通过，全量 CI pytest 265 passed、53 subtests、3 warnings，Node 3 passed、compileall/diff check 通过；已实际查看 1400×820 合成 NORMAL 46-40 预览；head `fe86c12`；CI `34171036388` 四项全绿；不访问真实账号或宣称真实联调/资源授权 |
| Campaign renderer asset lifecycle | `feat/campaign-resource-lifecycle-v2` | `bada0b3` | Draft PR #16；复用共享 AssetManager；显式传入的 falsey manager 不会被隐式替换；同一张卡片内相同 `(tid, resource_id)` 的 portrait 只解析一次，不同 `(tid, resource_id)` 不错误合并；本地 Campaign History 21 项 unittest 与 falsey manager 身份检查通过，完整 CI pytest 260 passed、43 subtests、3 warnings，Node 3 passed、compileall/diff check 通过；已实际查看共享 manager 生成的 1400×820 合成 Campaign 预览；head `d7f7d88`；CI `34171514608` 四项全绿；不宣称全局 N+1 已消除或真实资源联调 |
...
| Plugin shutdown lifecycle | `feat/plugin-shutdown-lifecycle` | `bada0b3` | Draft PR #21；顺序/并发 terminate 只回收一次资源；反馈、素材或 Web 回收异常时仍继续尝试后续资源，重新抛出首个异常且不设置完成标记，后续可重试；新增部分初始化缺失 Web、缺失任务注册表和 falsey 注入资源回收边界；最终 CI 全量 Python 3.10 为 261 passed、43 subtests、3 warnings，Python 3.11/3.12 与 Node 3 passed，compileall/diff check 通过；head `080ea94`；CI `34172309963` 四项全绿；当前环境 targeted wiring 因缺少 astrbot 依赖未本地执行；无 UI/渲染变化，不宣称现场或生产证据 |
| Data backup hardening | `feat/data-backup-hardening` | `bada0b3` | Draft PR #22；离线备份 `nikke.sqlite3` 与 `secret.key`，SQLite integrity_check、SHA-256 manifest、临时目录落盘和禁止覆盖；输出目录占用、损坏 SQLite 和源/输出路径符号链接等异常统一为 `BackupError`，备份密钥收紧为 `600`；本地 Python 3.10.11 专项 4 passed，compileall/diff check 通过；CI 全量 Python 3.11 报告 260 passed、43 subtests、3 warnings，Python 3.10/3.12 与 Node 全绿；head `bc839c4`；CI `34186296724` 四项全绿；仅合成数据，无真实恢复/部署声明 |
...
| Upgrade/rollback preflight | `feat/upgrade-preflight-v1` | `bada0b3` | Draft PR #30；只读检查 SQLite integrity/schema、数据库与 `secret.key` 成对存在、可选备份集和磁盘余量，输出 READY/MIGRATION_REQUIRED/BLOCKED；数据根目录、备份根目录及所有现有父级路径组件拒绝符号链接，不读取外部目标；当前本机缺少 pytest，未宣称本地 pytest，compileall/diff check 与父级链接 smoke 通过；CI `34188160651` 四项全绿（Python 3.10/3.11/3.12 均 265 passed、43 subtests，warnings 3/3/2；Node 3 passed）；head `e36d7b0`；不执行迁移/回滚/复制/删除/生产写入 |
| Final product review evidence matrix | `feat/requirement-evidence-matrix-v1` | `bada0b3` | Draft PR #31；覆盖路线图 27 个 `REQ-*`，区分主线代码/接线、Draft PR、离线测试、现场证据和人工授权；补充 `PARTIAL` 状态定义、逐行七列结构回归及产品状态枚举校验；本地直接执行 4 项通过、compileall/diff check 通过；CI `34188464410` 四项全绿（Python 3.10/3.11/3.12 均 260 passed、43 subtests，warnings 3/3/2；Node 3 passed）；head `3ce033a`；不执行真实账号/消息/部署/生产 migration/rollback |
| Tower static snapshot contract | `feat/tower-snapshot-contract-v1` | `bada0b3` | Draft PR #32；静态塔层快照在加载时校验来源标识、严格日期、来源 hash 文本、塔层键、正整数 `stage_id`/战力、唯一 stage ID 及重复 JSON 键拒绝；损坏快照安全失败，未知层明确未收录，异常或非规范输入不被静默归一化；CI `34188833142` 四项全绿（Python 3.10/3.11/3.12 均 260 passed、54 subtests，warnings 3/3/2；Node 3 passed）；当前本机缺少 pytest，未宣称本地 pytest，重复键 smoke、compileall/diff check 通过；head `f513b42`；未更新数据、访问来源、读取账号或宣称来源新鲜度/授权/玩家进度 |
| Character Data V2 registry and cache identity boundary | `feat/character-data-v2` | `bada0b3` | Draft PR #11；静态 Equipment/Cube/Favorite Item registry 保持精确 ID、hash 校验和重复 JSON 键拒绝；运行时拒绝布尔、浮点、空白和带符号 ID，未知 ID 不会通过 `sources.json`、同名本地缓存或远程请求显示资源；预览脚本可从仓库根目录直接运行并回收资源，已重新实际查看 red-hood/alice/fallback 离线合成卡；本地 Python 3.10.11 主题相关 unittest 27 passed，全量 CI pytest 262 passed、49 subtests、3 warnings、Node 3 passed、compileall/diff check 通过；head `57d292d`；CI `34170484699` 四项全绿；仍不宣称完整角色数值、真实账号字段、远程素材授权或产品合成联调 |
...
| Dynamic voice cache and lifecycle boundary | `feat/dynamic-voice-v2` | `bada0b3` | Draft PR #10；pipeline/provider 拒绝布尔、字符串、NaN、无穷和非正预算，pipeline 也拒绝非法 pending 容量；source cache 只有请求 `map_key`、`source_path`、SHA-256、MP3 头和非未来 24 小时有效期均匹配时才命中，缓存根目录/文件符号链接拒绝读写且不发起网络，关闭会取消在途共享下载并释放任务索引；本地 Python 3.10.11 Voice 专项 22 passed、12 subtests，全量 pytest 263 passed、55 subtests、Node 3 passed、compileall/diff check 通过；head `6fc6137`；CI `34191491292` 四项全绿；仍不宣称真实 Poke 映射、OneBot 播放、资源授权或账号联调 |
| Union Raid Increment A identifier boundary | `feat/union-raid-v2` | `bada0b3` | Draft PR #8；范围/HP/重复记录语义保持保守；overview 仅接受非空文本/明确整数 Boss ID，坏标识保留记录但标记未知覆盖并隐藏聚合；名称仅接受文本并安全回退，元素/图标/模型容器不会字符串化成伪标识；畸形元数据 Builder→Renderer 预览已实际查看；排名拒绝空白身份、空/非标 Boss ID 与角色 ID；head `4ad74c0`；本地定向 25 passed、16 subtests，全量 pytest 268 passed、59 subtests、Node 3 passed、compileall/diff check 通过；CI `34190392211` 四项全绿；分页完整性、canonical identity、历史赛季和“我的战斗”仍未宣称完成 |
| Announcement V2 source/record boundary | `feat/announcement-v2` | `bada0b3` | Draft PR #9；深度读取不回退旧源；稳定 ID 与核心文本字段严格校验；CMS 扫描上限、协议 code/result、content ID、标题和发布时间拒绝布尔/浮点/容器，保留公开 fixture 已证明的十进制字符串时间戳；版本字段严格为正整数，损坏缓存不静默截断；同步失败覆盖旧成功范围报告，仅记录安全错误类型且保留缓存；head `78b33a7`；本地主题回归 43 passed、54 subtests，全量 pytest 282 passed、73 subtests、Node 3 passed、compileall/diff check 通过；CI `34191009192` 四项全绿；未访问真实 CMS、账号或发送消息 |
