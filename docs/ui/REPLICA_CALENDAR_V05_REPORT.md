...
- 历史阶段交付记录。PR #103 后续将旧 classic/Pillow 单角色卡彻底退役：当前唯一生产路径为白色竖版 replica；本报告中关于 classic 或失败后使用 Pillow 的描述仅反映当时状态，不再实现。
...
- 当时由 `character_card_layout=replica` 选择案例竖版，失败后还会回退 Pillow；该双实现已经退役，当前角色卡不读取布局选项且始终使用白色竖版 T2I，失败返回明确错误。其他页面继续遵循 `ui_renderer`。
