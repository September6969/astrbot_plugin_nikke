...
| REQ-ANN-003 | 自动推送 | `/妮姬 账号 汇总 开|关`、后台公告订阅 | `announcement_delivery.py`、`main.py`；发送前持久化 `DISPATCH_INTENT`，发送异常/取消/游标提交失败进入 `UNKNOWN_AFTER_ACTION` 并禁止自动重发；明确 `False` 才进入受控失败退避；`tests/test_announcement_delivery.py`、`tests/test_announcement_push_wiring.py` 覆盖这些状态；`_dispatch_lock` 只提供单进程内互斥 | 未向真实群或用户发送消息；送达证据需明确授权；当前不支持没有外部共享租约/幂等协调的多实例公告推送 | `IMPLEMENTED / WIRED / SYNTHETIC_VERIFIED / READY_OFFLINE`；`NEEDS_LIVE_EVIDENCE` | 现场仅验证目标会话、频率和失败回收；未知投递必须人工/上游对账，不能自动重发；不能宣称 exactly-once，也不能把进程内锁称为跨实例锁；当前部署边界为单实例，多实例需另行增加协调方案 |
...
| REQ-VOICE-001 | Poke Voice | `/妮姬 戳一戳 [角色名]` | `voice_audio.py`、`voice_mapping.py`、`voice_pipeline.py`、`voice_resource_provider.py`、`voice_encoder.py`；PR #55 接入本地 → 证据映射动态资源 → 文本回退，Voice 专项行为测试与 Record 序列化测试覆盖预算、并发、取消、缓存、格式和生命周期；截至 2026-09-22 的 voice map 为 schema v3、2106 条 source-annotated 映射 | 未发送真实 QQ 音频；角色/皮肤映射的现场可用性、资源授权和实际播放未确认 | `IMPLEMENTED / WIRED / READY_OFFLINE`; `NEEDS_LIVE_EVIDENCE` | 仅在取得精确角色/服装/locale 映射和授权资源后做最小 NapCat/OneBot Record 现场验证，不采集秘密 |
