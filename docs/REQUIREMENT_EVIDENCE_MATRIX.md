...
| REQ-CDK-002 | 批量 CDK | `/妮姬 兑换 批量 <CDK...>`、`/妮姬 兑换 可用` | 主线有串行、账号锁、失败中止和输入去重测试；持久批量幂等的后续仍需按当前主线重新审计 | 不执行真实批量兑换，不读取真实历史 | `IMPLEMENTED / WIRED / SYNTHETIC_VERIFIED`；`NEEDS_HUMAN_DECISION` | 对照 `action_runs` 和 batch 语义补离线合同；真实消费必须单独授权 |
