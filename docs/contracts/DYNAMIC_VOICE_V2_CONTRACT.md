...
3. 缓存命中必须同时确认请求身份、内容完整性和非未来的 24 小时有效期；关闭时必须取消并释放共享下载任务。
...
- 只有目标文件、manifest、非未来的 24 小时有效期、SHA-256、MP3 头部，以及与当前请求相同的 `map_key` 和 `source_path` 同时有效时才命中持久缓存。
- `close()` 取消全部共享下载任务，等待其收敛并清空任务索引；关闭后不会创建新的下载任务。
