...
- InformationFeeds 的扫描上限、协议 `code/result`、content ID、标题和发布时间使用严格字段合同：布尔值不会冒充整数，容器不会字符串化成稳定 ID；已观察到的非负十进制 `pub_timestamp` 字符串继续兼容，浮点、带符号和溢出时间拒绝入库。
...
pytest tests/test_announcement_cache_lifecycle.py tests/test_announcement_v2.py tests/test_announcement_versions.py tests/test_announcements.py
                                                                 43 passed, 2 warnings, 54 subtests
python -m compileall -q .                                      PASS
node --test tests/extension.test.cjs                            3 passed
git diff --check                                                PASS
```

当前同一源码全量回归：`282 passed, 2 warnings, 73 subtests`；`compileall`、Node 扩展测试和 `git diff --check` 均通过。另以合成记录实际查看了语言/分类筛选文本与只读诊断输出：筛选只显示唯一维护公告；诊断只显示数量、范围、locale/category 统计和保留策略，不含正文、订阅目标或凭据。
