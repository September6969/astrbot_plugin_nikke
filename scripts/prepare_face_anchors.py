...
def main():
    ...
    def one(pair):
        ...
        try:
            ...
            subprocess.run(["node", str(ROOT / "scripts/extract_spine_face_anchor.mjs"), str(runtime),
                            str(skel), str(atlas), key, "idle", str(sidecar)], check=True, capture_output=True, timeout=45)
            ...
            with Image.open(png) as image:
                ...
                # 旧清单默认 1024 / 16px；新离线导出可显式登记等比高清画布。
                padding = entry.get("crop_padding", 16)
                render_width = entry.get("render_width", 1024)
                if type(padding) is not int or type(render_width) is not int or not 0 <= padding <= 256 or not 256 <= render_width <= 8192:
                    raise ValueError("无效的 PNG 变换")
                if rgba.size != (box[2] - box[0] + padding * 2, box[3] - box[1] + padding * 2):
                    raise ValueError("PNG 裁切变换与清单不一致，拒绝猜测坐标")
                ratio = render_width / 1024
                record["point"] = [record["point_1024"][0] * ratio - box[0] + padding, record["point_1024"][1] * ratio - box[1] + padding]
                record["extent"] = [value * ratio for value in record["extent_1024"]] if record["extent_1024"] else None
                ...
                record["png_transform"] = {"render_size": [render_width, render_width], "render_padding": .08, "alpha_bbox": box, "crop_padding": padding}
                record.pop("candidates")
                return key, record
    ...
    if args.output.is_file():
        old = json.loads(args.output.read_text(encoding="utf-8")).get("records", {})
    ...
    args.output.write_text(json.dumps({"schema": 1, "records": good, "unavailable": errors}, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({"prepared": len(good), "unavailable": errors}, ensure_ascii=False))
