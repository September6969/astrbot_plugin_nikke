...
from spine_core_axis import select_breast_anchor, select_head_top, validate_axis_order


def merge_records(old_records, rebuilt_records):
    """只替换本次清单记录，保留清单外已有 metadata。"""
...
def main():
        ...
        try:
            ...
            for ext in ("skel", "atlas"):
                ...
                if not path.is_file() and args.download_missing:
                    response = httpx.get(
                        f"https://raw.githubusercontent.com/Nikke-db/Nikke-db.github.io/main/l2d/{key}/{key}_00.{ext}",
                        timeout=20,
                    )
                    response.raise_for_status()
                    path.write_bytes(response.content)

            skel = args.bundle_dir / f"{key}.skel"
            atlas = args.bundle_dir / f"{key}.atlas"
            ...
            breast, breast_reason = select_breast_anchor(record.get("anatomy_bones_1024", []))
            head_top, head_reason = select_head_top(record.get("head_surface_candidates_1024", []))
            ...
            with Image.open(png) as image:
                ...
                # Sidecar-only discovery data must not leak into runtime metadata.
    ...
    # Preserve existing records outside this manifest.  The manifest is an
    # explicit rebuild scope, not permission to discard older verified rows.
    old_records = {}
    ...
    for key, record in good.items():
        ...
        for candidate in candidates:
            digest = candidate.get("pixel_sha256") if isinstance(candidate, dict) else None
            if digest and digest != record["pixel_sha256"]:
                variants[digest] = {
                    k: v for k, v in candidate.items() if k != "variants"
                }
        if variants:
