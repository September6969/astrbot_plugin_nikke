...
def main():
    ...
    try:
        renderer = CharacterCardRenderer(output, ROOT / "fonts")
        original = build_card()
        cards = {
            "red-hood": original,
        ...
        for name, card in cards.items():
            card_assets = manager.resolve_character_assets(card)
            generated = Path(renderer.render_character(card, card_assets))
