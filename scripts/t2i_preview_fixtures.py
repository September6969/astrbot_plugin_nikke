...
def calendar_cases(directory):
    ...
    for name in case_names:
        ...
        if name not in ("empty", "unavailable"):
            ...
            elif name == "landscape-kv-7-active-4-next":
                from PIL import Image
                for index in range(7):
                    start = NOW - timedelta(days=1)
                    end = NOW + timedelta(days=index + 2)
            ...
            elif name in ("landscape-kv-20-active", "landscape-kv-25-active"):
                from PIL import Image
                count = 20 if name == "landscape-kv-20-active" else 25
                for index in range(count):
                    start = NOW - timedelta(days=1)
                    end = NOW + timedelta(days=index + 2)
                    event = CalendarActivity(
...
def character_cases():
    ...
    result = {}
    for name, resource in (("c010", 10), ("c010_02", 10), ("c010_03", 10), ("c011", 11), ("c017", 17), ("c018", 18), ("c234", 234), ("c330", 330), ("c352", 352), ("c401", 401), ("c471", 471), ("c581", 581), ("representative-dark", 10), ("representative-light", 330), ("bright", 330), ("dark", 10), ("low-saturation", 352), ("wide-pose", 471),
