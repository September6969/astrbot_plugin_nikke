...
class NikkeCommandRuntime:
    ...
    def _call_entry(self, name: str, *args, **kwargs):
        """调用显式覆盖的旧入口，供迁移期测试与插件扩展保持兼容。"""
        method = vars(self.plugin).get(name)
        if method is None:
    ...
    async def nikke(
        ...
        if command_key in {"语音", "voice"}:
            async for result in self._call_entry("voice_settings", event, arg1, arg2):
                yield result
            return
        if command_key in {"塔罗", "tarot"}:
            async for result in self._call_entry("tarot_command", event, arg1, arg2):
                yield result
            return
        if command_key in {"", "帮助", "help"}:
            async for result in self._call_entry("nikke_help", event, arg1):
                yield result
            return
        if command_key in {"账号", "account"}:
            async for result in self._call_entry("account", event, arg1, arg2):
                yield result
            return
        if command_key in {"我的", "me", "progress"}:
            async for result in self._call_entry("me", event):
                yield result
            return
        if command_key in {"查询", "query"}:
            async for result in self._call_entry("query", event, arg1, arg2):
                yield result
            return
        if command_key in {"签到", "daily", "claim", "日常", "routine"}:
            async for result in self._call_entry("daily", event, arg1, arg2):
                yield result
            return
        if command_key in {"兑换", "cdk"}:
            ...
            if sub in {"批量", "batch"}:
                if not arg2:
                    ...
                    return
                async for result in self._call_entry("cdk_batch", event, arg2):
                    yield result
                return
            if sub in {"可用", "available"}:
                async for result in self._call_entry("cdk_available", event):
                    yield result
                return
            if sub in {"历史", "history"}:
                async for result in self._call_entry("cdk_history", event):
                    yield result
                return
            if not arg1:
                yield event.plain_result("用法：/妮姬 兑换 <CDK> 或 /妮姬 兑换 批量 <CDK...> 或 /妮姬 兑换 可用|历史")
                return
            async for result in self._call_entry("cdk", event, arg1):
                yield result
            return
        if command_key in {"战役", "campaign", "关卡", "stage"}:
            async for result in self._call_entry("campaign", event, arg1, arg2):
                yield result
            return
        if command_key in {"日程"}:
            try:
                stream = self._call_entry("event_schedule", event, arg1)
            except TypeError:
                stream = self._call_entry("event_schedule", event)
            async for result in stream:
                yield result
            return
        if command_key in {"schedule"}:
            if ":" in arg1 and self._is_admin(event):
                async for result in self._call_entry("schedule", event, arg1):
                    yield result
                return
            try:
                stream = self._call_entry("event_schedule", event, arg1)
        ...
        if command_key in {"公告", "news", "announcement"}:
            if arg1 in {"订阅", "取消订阅"}:
                async for result in self._call_entry("announcement_subscription", event, arg1):
                    yield result
                return
            if arg1:
                async for result in self.adapters.command.dispatch(
                    ...
                    yield result
                return
            async for result in self._call_entry("announcements_view", event):
                yield result
            return
        if command_key in {"攻略", "guide", "guides"}:
            async for result in self._call_entry("guide", event, arg1, arg2 or "1"):
                yield result
            return
        if command_key in {"突袭", "联盟突袭", "raid", "union_raid"}:
            if arg1 in {"排名", "ranking"}:
                async for result in self._call_entry("union_raid_ranking", event):
                    yield result
                return
            if arg1 in {"我的", "my"}:
                async for result in self._call_entry("union_raid_my", event):
                    yield result
                return
            async for result in self._call_entry("union_raid", event):
                yield result
            return
        if command_key in {"管理", "admin"}:
            async for result in self._call_entry("admin", event, arg1, arg2):
                yield result
            return
        ...
        legacy = {
            ...
            "status": ("status", (event,)),
            "roster": ("roster", (event,)),
            "character": ("character", (event, arg1)),
            "info": ("info", (event, arg1)),
            "campaign": ("campaign", (event, arg1, arg2)),
            "raid": ("union_raid", (event,)),
            "union_raid": ("union_raid", (event,)),
            "news": ("announcements_view", (event,)),
            "guide": ("guide", (event, arg1)),
            "push": ("push", (event, arg1)),
            "group": ("group_set", (event, arg1)),
            "schedule": ("schedule", (event, arg1)),
            "summary": ("summary", (event, arg1)),
            "run": ("run", (event,)),
            "health": ("health", (event,)),
            "tarot": ("tarot_command", (event, arg1, arg2)),
        }
        target = legacy.get(command_key)
        if target:
            name, args = target
            async for result in self._call_entry(name, *args):
                yield result
            return
    ...
    async def query(self, event: AstrMessageEvent, kind: str = "", name: str = ""):
        ...
        if kind_key in {"练度", "roster", "character"}:
            if name:
                async for result in self._call_entry("character", event, name):
                    yield result
            else:
                async for result in self._call_entry("roster", event):
                    yield result
            return
        if kind_key in {"资料", "info"}:
            if not name:
                ...
                return
            async for result in self._call_entry("info", event, name):
                yield result
            return
        if kind_key in {"战役", "关卡", "campaign", "stage"}:
            async for result in self._call_entry("campaign", event, name, ""):
                yield result
            return
        if kind_key in {"攻略", "guide"}:
            async for result in self._call_entry("guide", event, name):
                yield result
            return
