...
def test_summary_layout_is_bottom_anchored_for_zero_to_four_rows():
    heights = [summary_panel_height(count) for count in range(5)]
    assert heights[0] == 0
    assert heights[1] < heights[2] < heights[3] < heights[4]

    layouts = [summary_layout(count) for count in range(5)]
...
def test_unknown_summary_count_fails_closed():
    import pytest
...
def test_core_axis_clamp_protects_head_top_and_breast():
