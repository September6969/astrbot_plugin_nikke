...
"""Regression test for Eunhwa: Tactical Upgrade portrait resolution with costume TID 30049."""
...
def test_character_master_eunhwa_tactical_upgrade():
    ...
    assert char.default_costume_id == 30049
    ...
    # is_default_costume checks
    assert resolver.is_default_costume(95, 30049) is True
    assert resolver.is_default_costume(95, "30049") is True
    assert resolver.is_default_costume("c095", 30049) is True
...
def test_nikke_db_provider_resolves_eunhwa_default_costume(tmp_path):
    provider = NikkeDbProvider(tmp_path, ASSETS_DIR)

    # Base costume 30049 maps to c095
    assert provider.resolve_render_id(95, 30049) == "c095"
    assert provider.resolve_render_id(95, "30049") == "c095"
    assert provider.resolve_render_id("95", 30049) == "c095"
    assert provider.resolve_render_id("c095", 30049) == "c095"
    assert provider.resolve_character_id(95, 30049) == "c095"
    assert provider.costume_cache_token(30049, resource_id=95) == ("default", "default")

    # None, 0, "0", "default" also map to c095
    ...
    # Non-default invalid costumes must return "missing"
    ...
    # Known alternate costume: Marian Racer's High (110023 -> c222_01)
...
def test_asset_manager_portrait_resolution_with_manifest(tmp_path):
    ...
    # Create dummy rendered PNG for c095
    dummy_img = Image.new("RGBA", (504, 892), (255, 0, 0, 255))
    c095_path = rendered_dir / "c095.png"
    dummy_img.save(c095_path)
    ...
    # Portrait call with costume_id=30049
    portrait = manager.get_character_portrait(5149, 95, 30049)
    assert portrait is not None
    assert portrait.size == (504, 892)
    # Ensure it's not the grey fallback placeholder (which is 128x128)
    assert portrait.size != (128, 128)

    # Also check resolve_character_assets
    ...
    assert resolved.portrait.size == (504, 892)
    manager.close()
