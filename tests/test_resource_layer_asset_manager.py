...
class ResourceLayerAssetManagerTests(unittest.TestCase):
    ...
    def test_skill_icon_remote_success_and_cache_writing(self):
        with tempfile.TemporaryDirectory() as td:
            cache_dir = Path(td)
            manager = AssetManager(cache_dir, self.assets_dir, remote=True)
            try:
                # 构造合法 PNG 字节流模拟远程成功响应
                mock_png = io.BytesIO()
                ...
                with patch("astrbot_plugin_nikke.asset_manager.httpx.stream") as mock_stream:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                with patch("astrbot_plugin_nikke.asset_manager.httpx.stream") as mock_stream:
                    ...
                    img = manager.get_skill_icon(102, "s1")
                    # 解码失败，应安全降级到 S1 fallback，且不把损坏内容写入缓存
                    self.assertEqual(img.size, (128, 128))
                    cached_file = cache_dir / "skills" / "generic" / "icn_skill_statreloadtime_01.png"
