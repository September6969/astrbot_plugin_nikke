...
async def test_handler_fallback_same_dto_without_second_fetch(failure):
    ...
    record = preview.fixture_record("normal")
    plugin.campaign_builder = Mock()
    plugin.campaign_builder._directory_by_tid = {}
    plugin.campaign_builder.build.return_value = record
    plugin.campaign_resolver = Mock()
    plugin.campaign_resolver.resolve_query.return_value = Mock(stage_id=record.stage_id)
    plugin._directory = []
    plugin._account_or_error = Mock(return_value={})
    plugin.client = Mock(get_main_quest_clear_lineup=AsyncMock(return_value={}))
    ...
    plugin.campaign_t2i_renderer.render_campaign_history.assert_awaited_once_with(record)
