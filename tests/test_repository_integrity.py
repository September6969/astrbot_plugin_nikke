...
class RepositoryIntegrityTests(unittest.TestCase):
    ...
    def test_root_contains_only_required_py_files(self):
        """验证根目录下仅存在 4 个正式入口 .py 文件。"""
        py_files = sorted([p.name for p in ROOT.glob("*.py")])
        expected = ["__init__.py", "_version.py", "container.py", "main.py"]
        self.assertEqual(py_files, expected, f"根目录下存在非预期的 .py 文件: {py_files}")
