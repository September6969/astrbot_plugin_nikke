...
if [ -f "$lock_file" ]; then
    ...
    if [ -n "$existing_pid" ] && kill -0 "$existing_pid" 2>/dev/null; then
        ...
        exec /usr/local/bin/nikke-spine-worker-4.1 "$@"
    else
        rm -f "$lock_file" "/tmp/.X11-unix/X${disp_num}" 2>/dev/null || true
    fi
fi

echo "nikke-spine-worker-4.1: 启动 Xvfb ${display}" >&2
...
export DISPLAY="${display}"
echo "nikke-spine-worker-4.1: Xvfb 已就绪，启动 worker" >&2

worker_ret=0
/usr/local/bin/nikke-spine-worker-4.1 "$@" || worker_ret=$?
