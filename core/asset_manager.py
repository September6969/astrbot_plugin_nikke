...
class AssetManager:
    ...
    def fallback(cls, kind: str) -> Image.Image:
        ...
        fallback_dir = (Path(__file__).resolve().parents[1] / "assets" / "fallback") if (Path(__file__).resolve().parents[1] / "assets" / "fallback").exists() else (Path(__file__).resolve().parent / "assets" / "fallback")
